...
const MainHeader = () => {
  const { itemCount, cartTotal, cart, removeFromCart, updateQuantity } = useCart();
              ...
              <ScrollArea className="flex-grow p-6">
                ...
                <div className="grid grid-cols-1 gap-2">
                  <Link to="/cart" className="block w-full" onClick={() => setIsSearchFocused(false)}>
                    <Button variant="outline" className="h-12 w-full text-[10px] font-bold uppercase rounded-xl border-2 hover:bg-secondary transition-all shadow-sm">
                      View Full Cart
                    </Button>
                  </Link>
                  <Link to="/checkout" className="block w-full">
                    <Button className="h-14 w-full text-sm font-bold uppercase rounded-2xl bg-primary text-primary-foreground shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all" disabled={cart.length === 0}>
                      Proceed to checkout
                    </Button>
                  </Link>
                </div>
                <p className="text-[10px] text-center font-bold text-muted-foreground">Secure checkout via SSL</p>
              </div>
            </SheetContent>
          </Sheet>
  ...
  <div className="bg-background border-b py-3 hidden lg:block sticky top-[89px] z-40">
    <div className="max-w-[1140px] mx-auto px-4 flex items-center justify-between">
      ...
      <div className="flex items-center gap-4">
        <Link to="/track" className="text-xs font-bold border rounded-full px-4 py-1.5 hover:bg-secondary transition-colors uppercase">Track Order</Link>
      </div>
    </div>
  </div>
);
